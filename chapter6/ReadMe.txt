...
Introducing EF
    ...
    Adding nuget package - EF Core (Microsoft.EntityFrameworkCore)
    Add a class BugsContext.cs and inherit from dbContext
    Add DbSets
		Override on model creating
	Configure DbContext options in startup
		Going to use In memory database
        Add nuget package - EF Core in memory (Microsoft.EntityFrameworkCore.InMemory)

    In controller, get action methods => get original data from database
        Test if Tickets properly coming from the database
